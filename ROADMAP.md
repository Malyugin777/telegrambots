...
### 7.1 Research APIs (Phase 1) ✅ ИССЛЕДОВАНО

**Задача:** Найти API которые РЕАЛЬНО проксируют контент через свой CDN, а не возвращают googlevideo.com URLs.

| API | Тип | CDN | Цена | Статус | Примечания |
|-----|-----|-----|------|--------|------------|
| **youtube-download-api.org** | Proxy CDN | ✅ `media.yt-data-proxy.org` | $199/мес | 🏆 ЛУЧШИЙ | Premium proxy network, <2 сек ответ |
| **SaveNow.to** | Pay-per-download | ❓ Неясно | ~$0.00025-0.00150/download | ⚠️ ПРОВЕРИТЬ | Progress tracking, multiple URLs |
| **Cobalt.tools** | Self-hosted | ❌ Заблокирован | Нужен residential proxy | ❌ НЕ ПОДХОДИТ | Main instance забанен YouTube |
| **RapidAPI (текущий)** | Direct URL | ❌ googlevideo.com | ~$0.004/download | ⚠️ Бан IP | Возвращает прямые YouTube URLs |

**Результаты исследования (2026-01-20):**

✅ **youtube-download-api.org** — РЕКОМЕНДУЕТСЯ
- Возвращает URL их CDN: `https://media.yt-data-proxy.org/...`
- НЕ googlevideo.com → наш IP не банится
- <2 сек на получение ссылки
- $199/мес flat rate (без лимитов?)
- **Следующий шаг:** запросить trial/тест

⚠️ **SaveNow.to** — НУЖЕН ТЕСТ
- Pay-per-download: $0.00025 (short) — $0.00150 (long 3x)
- 10K downloads/day = $2.50-$15/day
- Неясно, проксирует ли через CDN
- **Следующий шаг:** тест на реальном видео, проверить host

❌ **Cobalt.tools** — НЕ ПОДХОДИТ
- Main instance заблокирован YouTube с mid-2025
- Self-hosted требует residential proxy ($$$)
- Слишком сложно для нашего масштаба

**Критерии оценки:**
- [x] Проксирует через свой CDN (НЕ googlevideo.com) → youtube-download-api.org ✅
- [ ] Скорость скачивания >5 MB/s → нужен тест
- [ ] Поддержка длинных видео (>60 мин) → нужен тест
- [x] Цена/лимиты → youtube-download-api.org $199/мес, SaveNow pay-per-use
