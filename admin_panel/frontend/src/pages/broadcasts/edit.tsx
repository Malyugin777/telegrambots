...
export const BroadcastEdit = () => {
  ...
  // Sync targetType with form data when loaded
  useEffect(() => {
    if (queryResult?.data?.data?.target_type) {
      setTargetType(queryResult.data.data.target_type);
    }
  }, [queryResult?.data?.data?.target_type]);
